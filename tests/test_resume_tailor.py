...
def test_update_work_exp_success(base_resume):
    updated_work_exp = [
    ...
    result = tailor.update_work_exp(updated_work_exp)
    assert result is True
    assert base_resume["data"]["work_experience"][0]["responsibilities"] == ["New task A"]

def test_update_work_exp_mismatched_length(base_resume):
    updated = [{"title": "Engineer", "responsibilities": ["New task A"]}]
    tailor = ResumeTailor(base_resume, BeautifulSoup("", "html.parser"))
    result = tailor.update_work_exp(updated)
